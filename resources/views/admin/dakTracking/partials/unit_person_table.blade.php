     @foreach ($shipments as $key => $shipment)
         @if (in_array($shipment->barcode, session('scanned_barcodes', [])))
             <tr>
                 <td><input type="checkbox" name="shipment_ids[]" value="{{ $shipment->id }}" class="shipment-checkbox">
                 </td>
                 <td>{{ ++$key }}</td>
                 ...
                 <td>
                    ...
                    <i class="bi bi-person"></i>
                     ...
                     <form action="{{ route('admin.tracking.cancel', $shipment->barcode) }}" method="POST"
